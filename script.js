(() => {
  ...
  const api = new JitsiMeetExternalAPI("meet.jit.si", {
    roomName: `PetCam-${room}`,
    parentNode: document.querySelector("#app"),
    width: "100%",
    height: "100%",
    configOverwrite: {
      prejoinPageEnabled: false,
      startWithAudioMuted: isTablet,
      startWithVideoMuted: isTablet,
      disableDeepLinking: true,
      enableWelcomePage: false
    },
    interfaceConfigOverwrite: {
      DEFAULT_BACKGROUND: "#080b0d",
      MOBILE_APP_PROMO: false,
      SHOW_JITSI_WATERMARK: false,
      SHOW_WATERMARK_FOR_GUESTS: false,
      TOOLBAR_BUTTONS: isTablet
        ? ["microphone", "camera", "hangup", "settings"]
        : ["microphone", "camera", "hangup", "settings", "tileview", "fullscreen"]
    },
  ...
  api.addEventListener("videoConferenceJoined", () => {
    status.textContent = isTablet ? "Waiting for your iPhone…" : "Connected.";
  });
  ...
  api.addEventListener("participantLeft", () => {
    ...
    if (visitorCount === 0) setTabletLive(false);
  });
})();
